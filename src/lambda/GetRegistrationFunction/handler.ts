import { getInvitationAttachment, isParticipantGroup } from '../../lib/registration'
...
const getRegistrationLambda = lambda('getRegistration', async (event) => {
  ...
  if (isParticipantGroup(registration.group?.key)) {
    registration.invitationAttachment = getInvitationAttachment(dogEvent, registration)
  }
