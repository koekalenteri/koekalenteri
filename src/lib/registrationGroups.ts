...
export const applyRegistrationGroupMoves = <T extends GroupedRegistration>(
  ...
  for (const move of moves) {
    ...
    const destination = { ...move.group } as T['group']
    ...
    item.group = {
      ...destination,
      // Preserve command order when multiple batch moves share an anchor.
      // Each fractional position remains before the anchor while increasing
      // toward it, so sortSnapshot keeps earlier commands first.
