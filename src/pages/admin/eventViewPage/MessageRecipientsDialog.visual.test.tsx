...
describeInLanguage('en', () => {
  it('names the recipient groups of every class', async () => {
    const screen = await renderDialog()

    await expect.element(screen.getByText('Who is the message for?')).toBeVisible()
    await expect(screen.getByRole('dialog')).toMatchScreenshot('message-recipients-en')
  })
})
