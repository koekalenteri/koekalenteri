...
it('names the part of the fee still missing', async () => {
  const screen = await renderPartMissing()
...
// The guide's English page shows the box in English (KOE-1437).
describeInLanguage('en', () => {
  it('names the part of the fee still missing', async () => {
    const screen = await renderPartMissing()

    await expect.element(screen.getByText(/of the fee is still missing/)).toBeVisible()
    await expect(screen.getByTestId('visual-root')).toMatchScreenshot('info-box-part-missing-en')
  })
})
