...
`)
    ...
    it('should render helperText', () => {
      const { container } = render(<AutocompleteSingle id='test-helperText' options={['test-a', 'test-b']} label={'test-label'} helperText={'helper text'} />)
      expect(container).toMatchSnapshot()
    })

    it('should render helperText with error state', () => {
      const { container } = render(<AutocompleteSingle id='test-helperTest-error' options={['test-a', 'test-b']} label={'test-label'} helperText={'helper text'} error />)
      expect(container).toMatchSnapshot()
    })
    ...
    it.each([undefined, null, '', 'test-a', 'test-b', 'test-c'])('should not call onChange on initial render when value=%p', async (value) => {
      const onChange = jest.fn()
      render(<AutocompleteSingle id='test-warn' options={['test-a', 'test-b']} label={'test-label'} value={value} onChange={onChange} />)
